...
class SlurmCluster(Cluster):
    ...
    def connect(self):
        ...
        if not host:
            raise ValueError(f"Slurm cluster {self.name} missing 'host' config")
        ...
        logger.info(f"Connecting to Slurm login node '{self.name}' at {resolved_host}:{resolved_port}...")
        try:
            ...
            # Check if sinfo is available to verify it's a slurm node
            stdin, stdout, stderr = self.ssh_client.exec_command("sinfo --version")
            exit_status = stdout.channel.recv_exit_status()
            if exit_status == 0:
                logger.info(f"Successfully connected to Slurm cluster '{self.name}'.")
            else:
                err = stderr.read().decode('utf-8')
                raise RuntimeError(f"Slurm verification failed: {err}")
        except Exception as e:
            logger.error(f"Failed to connect to or verify Slurm on '{self.name}': {e}")
            raise

    def spin_up(self, service_name: str, command: str):
        logger.info(f"Spinning up service '{service_name}' on Slurm cluster '{self.name}'...")
        try:
