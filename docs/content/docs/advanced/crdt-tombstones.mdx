...
description: How OpenTela uses explicit tombstones and a StatusLeft mechanism to reliably handle node departures in the CRDT-based node table.
---
...
```bash
go test -v -run TestNodeLeave opentela/internal/protocol
```
