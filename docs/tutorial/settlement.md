...
- A Solana wallet with OTELA tokens (see [Wallet & Ownership](owner.md))
...
## Step 2: Configure the OTELA token

The settlement system needs to know which SPL token to use for payments. The default configuration points to the OTELA token on devnet:
...
If you are running on a different cluster or using a custom token, update the `mint_address` and `decimals` accordingly.

## Step 3: Configure the Solana RPC endpoint

```yaml
solana:
  rpc_endpoint: "https://api.devnet.solana.com"
  priority_fee: 1000  # microlamports (optional, for faster confirmation)
```
...
RPC:    https://api.devnet.solana.com
  ...
  Token (BAYyKYocgUgWHcs25Sar15x9XY9iYdtnKurBoVXJD1bU): 50000 (50000.000000000)
```
...
Start worker nodes as usual (see [Spin Up](spinup.md)). The billing system operates transparently — worker nodes do not need any special configuration beyond having a wallet.
...
1. Point `solana.rpc_endpoint` to `https://api.devnet.solana.com`
2. Request SOL from the faucet: `./otela wallet airdrop 2`
3. Deploy or use an existing test SPL token on devnet
4. Update `otela_token.mint_address` to the test token's mint
